...
@bp.route(f'/probes/{__plugin_name}/<probe_name>/reset', methods=['POST'])
def reset_rules(probe_name: str) -> str:
    ...
    """
    try:
        return jsonify(Controller().get_probe(__plugin_name, probe_name).reset())
    except exceptions.ProbeNotFoundException as e:
        abort(404, e)


@bp.route(f'/probes/{__plugin_name}/<probe_name>/rules', methods=['GET', 'POST', 'DELETE'])
def manage_rules(probe_name: str) -> Union[List[MitigatorRule], str]:
    ...
    except (exceptions.ProbeNotFoundException, MemoryError, IndexError, LookupError) as e:
        abort(404, e)


@bp.route(f'/probes/{__plugin_name}/<probe_name>/rules/<int:rule_id>', methods=['GET', 'DELETE', 'PUT', 'POST'])
def manage_rule_at(probe_name: str, rule_id: int) -> Union[MitigatorRule, str]:
    ...
    try:
        ...
        rule = MitigatorRule(**request.json)
        return jsonify(probe.insert_at(rule_id, rule) if request.method == 'POST'
                       else probe.update(rule_id, MitigatorRule(rule)))
    except (exceptions.ProbeNotFoundException, MemoryError, IndexError, LookupError) as e:
        abort(404, e)
